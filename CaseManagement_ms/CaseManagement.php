	...
	<table class="table table-bordered border-dark w-100" id="db_table" style="min-width:1200px;">
		<thead class="table-light border-dark">
			<tr style="border-bottom: 1px solid #000;">
				...
				<th class="text-center text-nowrap" style="width:12%;padding: 10px;background-color: #CBF3FC;">上包-建商名稱</th>
				<th class="text-center text-nowrap" style="width:12%;padding: 10px;background-color: #CBF3FC;">上包-營造廠名稱</th>
				<th class="text-center text-nowrap" style="width:10%;padding: 10px;background-color: #CBF3FC;">連絡人</th>
				<th class="text-center text-nowrap" style="width:18%;padding: 10px;background-color: #CBF3FC;">案場位置</th>
...
<script type="text/javascript" charset="utf-8">
	...
	$(document).ready(function() {
		$('#db_table').dataTable( {
			...
			"ordering": false,
			"ajaxSource": "/smarty/templates/$site_db/$templates/sub_modal/project/func01/CaseManagement_ms/server_CaseManagement.php?site_db=$site_db&fm=$fm",
			"language": {
						"sUrl": "$dataTable_de"
			...
			"fnRowCallback": function( nRow, aData, iDisplayIndex ) { 
				...
				//	engineering_name = '<span class="size12 weight me-1 text-nowrap">'+aData[5]+'</span>';
				...
				if (aData[6] != null && aData[6] != "")
					builder_id = '<span class="size09">'+aData[6]+'</span>';
				...
				if (aData[7] != null && aData[7] != "")
					contractor_id = '<span class="size09">'+aData[7]+'</span>';
				...
				$('td:eq(7)', nRow).html( '<div class="d-flex justify-content-center align-items-center text-center" style="height:auto;min-height:32px;">'+contact+'</div>' );

				//案場位置
				/*
				var site_location = "";
				if (aData[9] != null && aData[9] != "")
					site_location = aData[9];
				*/
				var zipcode = '';
				...
				//$('td:eq(8)', nRow).html( '<div class="d-flex justify-content-center align-items-center text-center size12" style="height:auto;min-height:32px;">'+site_location+'</div>' );
				...
				if (aData[10] != null && aData[10] != "")
					ContractingModel = aData[10];

				$('td:eq(9)', nRow).html( '<div class="d-flex justify-content-center align-items-center text-center text-nowrap" style="height:auto;min-height:32px;">'+ContractingModel+'</div>' );

				//所屬公司
				var company_name = "";
				if (aData[26] != null && aData[26] != "") {
					company_name = '<span class="size12 me-1 text-nowrap">'+aData[26]+'</span>';
				} else {
					if (aData[25] != null && aData[25] != "")
						company_name = '<span class="size12 me-1 text-nowrap">'+aData[25]+'</span>';
				}

				var company_id = "";
				if (aData[24] != null && aData[24] != "")
					company_id = '<span class="size09">'+aData[24]+'</span>';

				$('td:eq(10)', nRow).html( '<div class="d-flex justify-content-center align-items-center text-center" style="height:auto;min-height:32px;">'+company_name+company_id+'</div>' );
				*/
				...
				if (aData[13] != null && aData[13] != "")
					last_modify = '<div class="text-nowrap">'+moment(aData[13]).format('YYYY-MM-DD HH:mm')+'</div>';
				
				//編輯人員
				var member_name = "";
				if (aData[18] != null && aData[18] != "")
					member_name = '<div class="text-nowrap">'+aData[18]+'</div>';

				$('td:eq(10)', nRow).html( '<div class="text-center" style="height:auto;min-height:32px;">'+last_modify+member_name+'</div>' );
